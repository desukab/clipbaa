...
def _parse_shopify_product(
    response: Response,
    category: str,
) -> Optional[DeodapProduct]:
    """Parse a DeoDap (Shopify) product page: {sku, wholesale price, mrp, images, description}."""
...
def run_deodap_spider(categories: Optional[List[str]] = None, use_mock: bool = False) -> List[dict]:
    if use_mock:
        mock_categories = resolve_categories(categories)
        items = [asdict(p) for p in MOCK_DEODAP_CATALOG if p.category in mock_categories]
        return write_items(items, "data/deodap_catalog_raw.json")
    items = run_spider(DeodapSpider, max_products=0, extra={"categories": categories})
    return write_items(items, "data/deodap_catalog_raw.json")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DeoDap wholesale catalog spider")
    parser.add_argument("--mock", action="store_true", help="Use bundled mock data instead of live scraping")
    parser.add_argument("--categories", type=str, default=None, help="Comma-separated category slugs (default: all)")
    args = parser.parse_args()
    categories = [c.strip() for c in args.categories.split(",")] if args.categories else None
    run_deodap_spider(categories=categories, use_mock=args.mock)