...
def run_flipkart_spider(max_products: int = 25, use_mock: bool = False) -> List[dict]:
    if use_mock:
        items = [asdict(p) for p in MOCK_FLIPKART_BESTSELLERS[:max_products]]
        return write_items(items, "data/flipkart_bestsellers_raw.json")
    items = run_spider(FlipkartSpider, max_products=max_products)
    return write_items(items, "data/flipkart_bestsellers_raw.json")


def fetch_bestsellers_page() -> List[str]:
    return [p.product_id for p in MOCK_FLIPKART_BESTSELLERS]


def fetch_product(product_id: str) -> Optional[FlipkartProduct]:
    for p in MOCK_FLIPKART_BESTSELLERS:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Flipkart Bestsellers spider")
    parser.add_argument("--mock", action="store_true", help="Use bundled mock data instead of live scraping")
    parser.add_argument("--limit", type=int, default=25, help="Max products to scrape (default: 25)")
    args = parser.parse_args()
    run_flipkart_spider(args.limit, use_mock=args.mock)