...
from scrapers.base import MarketplaceSpider, run_spider, write_items
...
def load_seed_asins() -> List[str]:
    ...
    try:
        ...
        if asins:
            print(f"  Loaded {len(asins)} seed ASINs from seed_asins.json")
        return asins
...
def run_movers_spider(max_products: int = 50, use_mock: bool = False) -> List[dict]:
    if use_mock:
        items = [asdict(p) for p in MOCK_AMAZON_MOVERS[:max_products]]
        return write_items(items, "data/amazon_movers_raw.json")
    items = run_spider(AmazonMoversSpider, max_products=max_products)
    return write_items(items, "data/amazon_movers_raw.json")


def fetch_movers_page() -> List[str]:
    return [p.asin for p in MOCK_AMAZON_MOVERS]


def fetch_product(asin: str, movers_rank: int) -> Optional[MoversProduct]:
    for p in MOCK_AMAZON_MOVERS:
        if p.asin == asin:
            return MoversProduct(**{**asdict(p), "movers_rank": movers_rank})
    return None
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Amazon Movers & Shakers spider")
    parser.add_argument("--mock", action="store_true", help="Use bundled mock data instead of live scraping")
    parser.add_argument("--limit", type=int, default=50, help="Max products per category to scrape (default: 50)")
    parser.add_argument("--no-enrich", action="store_true", help="Skip per-product /dp/ detail page enrichment")
    args = parser.parse_args()

    if args.no_enrich:
        AmazonMoversSpider.enrich = False
    run_movers_spider(args.limit, use_mock=args.mock)