...
def run_meesho_spider(max_products: int = 30, use_mock: bool = False) -> List[dict]:
    if use_mock:
        items = [asdict(p) for p in MOCK_MEESHO_TRENDING[:max_products]]
        return write_items(items, "data/meesho_trending_raw.json")
    items = run_spider(MeeshoSpider, max_products=max_products)
    return write_items(items, "data/meesho_trending_raw.json")


def fetch_trending_page() -> List[str]:
    return [p.product_id for p in MOCK_MEESHO_TRENDING]


def fetch_product(product_id: str) -> Optional[MeeshoProduct]:
    for p in MOCK_MEESHO_TRENDING:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Meesho Trending spider")
    parser.add_argument("--mock", action="store_true", help="Use bundled mock data instead of live scraping")
    parser.add_argument("--limit", type=int, default=30, help="Max products to scrape (default: 30)")
    args = parser.parse_args()
    run_meesho_spider(args.limit, use_mock=args.mock)