...
def run_spider(
    ...
    crawldir = os.environ.get(CRAWL_DIR_ENV)
    kwargs: Dict[str, Any] = dict(max_products=max_products) if spider_cls.max_products else {}
    kwargs.update(extra or {})
    spider = spider_cls(crawldir=crawldir, **kwargs)
    print(f"  Running {spider.name} (adaptive={'on' if spider.flags['adaptive'] else 'off'})...")
    result = spider.start()
    items = dedupe_items([item for item in result.items])
    print(f"  {spider.name}: scraped {len(items)} items")
    if not items:
        print(f"  [FAIL] {spider.name} produced no items")
    return items
